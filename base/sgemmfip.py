...
def sgemmfip_mcxnr_getref(
    ...
    def sgemmfip_mcxnr_ref(
        num_mr: size,
        alpha: f32[1] @ DRAM,
        C: f32[num_mr * mr, nr] @ DRAM,
        A: f32[num_mr * mr, kc] @ DRAM,
        B: f32[kc, nr] @ DRAM,
        beta: f32[1] @ DRAM,
        Abuffer: f32[mr, num_mr * kc] @ DRAM,
        Bbuffer: f32[nr, kc] @ DRAM,
    ):
            ...
            for l in seq(0, kc):
                for jr in seq(0, nr):
                    for ir in seq(0, mr):
                        if a_packed:
                            if b_packed: # or ic > 0:
                                C[ic * mr + ir, jr] += Abuffer[ir, l + ic * kc] * Bbuffer[jr, l] 
                            else:
                                C[ic * mr + ir, jr] += Abuffer[ir, l + ic * kc] * B[l, jr] 
                                Bbuffer[jr, l] = B[l, jr]
                        else:
                            if b_packed: # or ic > 0:
                                C[ic * mr + ir, jr] += A[ic * mr + ir, l] * Bbuffer[jr, l] 
                            else:
                                C[ic * mr + ir, jr] += A[ic * mr + ir, l] * B[l, jr] 
                                Bbuffer[jr, l] = B[l, jr]
                            Abuffer[ir, l + ic * kc] = A[ic * mr + ir, l]
    return proc(sgemmfip_mcxnr_ref)


def generate_sgemm(kc, mr, nr, a_packed, b_packed):
    p = simplify(sgemmfip_mcxnr_getref(kc, mr, nr, a_packed, b_packed))
    # p = p.partial_eval(mr, nr, a_packed, b_packed)
    # print(p)

    p = rename(p, "sgemmfip_{}x{}x{}_{}{}".format(mr, nr, kc, a_packed, b_packed))
